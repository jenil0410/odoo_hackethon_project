...
@section('content')
    <div class="card h-100">
        <div class="card-header d-flex align-items-center justify-content-between py-3">
            <h5 class="card-title m-0 me-2 text-white">Update User</h5>
        </div>
        <form action="{{ route('user.update', $user->id) }}" method="post" enctype="multipart/form-data" id="userUpdate" data-parsley-validate>
            @csrf
            @method('PUT')
                    ...
                    <div class="col-lg-4 col-md-6 col-12">
                        <div class="form-floating form-floating-outline mb-4">
                            <input autocomplete="off" type="text" class="form-control" name="first_name"
                                value="{{ old('first_name', $user->first_name) }}" id="first_name" required
                        ...
                        <div class="form-floating form-floating-outline mb-4">
                            <select class="form-select select2" id="role_id" name="role_id" required
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->id }}" @selected(old('role_id', optional($user->role)->id) == $role->id)>
                                        {{ $role->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-lg-4 col-md-6 col-12">
                        <div class="form-floating form-floating-outline mb-4">
                            <input autocomplete="off" type="text" class="form-control flatpickr-date" name="date_of_birth"
                                value="{{ old('date_of_birth', optional($user->date_of_birth)->format('Y-m-d')) }}" id="date_of_birth" placeholder="YYYY-MM-DD"
                    ...
                    <div class="col-lg-4 col-md-6 col-12">
                        <div class="mb-4">
                            ...
                            @if ($user->profile_photo)
                                <small class="d-block mt-1">Current: {{ $user->profile_photo }}</small>
                            @endif
                            @error('profile_photo')
                                <small class="red-text ml-10" role="alert">{{ $message }}</small>
                            @enderror
...
@section('scripts')
    ...
    <script>
        $(function() {
            var userUpdateForm = $('#userUpdate').parsley({
